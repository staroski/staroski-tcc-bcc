...
public class LocalDevice {
    ...
    public String getFriendlyName() {
        try { return bluetoothManager.getLocalDeviceName(); }
        catch (Exception e) { System.err.println("Exception: LocalDevice.getFriendlyName(): " + e); }
        return null;
    }
    ...
    public DeviceClass getDeviceClass() {
        ...
        catch (Exception e) { System.err.println("HCIException: LocalDevice.getDeviceClass(): " + e); }
        return null;
    }
    ...
    public static String getProperty(String property) {
      ...
      else if (property.equals("bluetooth.sd.attr.retrievable.max")) return null;

      else if (property.equals("bluetooth.l2cap.receiveMTU.max")) return null;

      else {
        try {
        if (property.equals("bluetooth.master.switch")) return ""+BlueZ.isMasterSwitchAllowed();
        if (property.equals("bluetooth.connected.inquiry.scan")) return ""+BlueZ.inquiryScanAndConAllowed();
        if (property.equals("bluetooth.connected.page.scan")) return ""+BlueZ.pageScanAndConAllowed();
        if (property.equals("bluetooth.connected.inquiry")) return ""+BlueZ.inquiryAndConAllowed();
        if (property.equals("bluetooth.connected.page")) return ""+BlueZ.pageAndConnAllowed();
    ...
    public String getBluetoothAddress() {
        try {
            if (bdAddrString == null) bdAddrString = bluetoothManager.getLocalDeviceAddress();
            return bdAddrString;
        }
        catch (Exception e) {
        ...
        for(int i=0;i<v.size();i++) {
          try {
            ...
            if(data.getDataType()==DataElement.UUID) {
              ...
              if(value.toLong()==0x1101) found=true;
            }
          }catch(Exception ex) {throw new Exception("The service class ID must be a DataElement of DataElements!");}
        }
